---
title: "既是自编码器，也是RNN，DeepMind科学家八个视角剖析扩散模型"
slug: "既是自编码器-也是rnn-deepmind科学家八个视角剖析扩散模型"
description: "作者： 机器之心 来源： 机器之心 机器之心编译 作者：Sander Dieleman 编辑：Panda W 扩散模型很火，对其描述也大有不同。本文中，一位 DeepMind 研究科学家全面剖析了「扩散模型是什么」这一课题。 如果你尝试过目前最火的 AI 绘画工具之一 Stable Diffusion，那你就已经体验过扩散模型（diffusion model）那强大..."
category: "ai-news"
tags: ["chatgpt", "AI", "AI聊天", "AI文本生成", "AI绘画", "AI编程", "AI电商"]
author: "AiBard123"
sourceUrl: "https://mp.weixin.qq.com/s/LOhX3BzPdigW1grzXQKmdg"
sourceName: "机器之心"
readingTime: "25 min"
...
作者： 机器之心  来源： [机器之心](https://mp.weixin.qq.com/s/LOhX3BzPdigW1grzXQKmdg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLphVkHz1ZwI2ckIXq1XSpRBBrvxWRQx9Uv8k2RwMNhZRjbsXNTMImM0A/640?wx_fmt=png)
*去噪自动编码器（左）和扩散模型（右）的示意图*
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLpoWWAVQmqaJVficcVicPmyMH6X7DP6kQicNEW8XWXDKN6YyMkFoHWdV9zg/640?wx_fmt=png)
*DDPM 示意图*
...
研究表明，还有另一种拟合分布的方法可以巧妙地避开对归一化的要求，即分数匹配（score matching）。这基于这一观察：所谓的分数函数（score matching）![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLpoQ0my5vmVibOHXDA54BSMcCrIhsco6jw5gicTvSWOib7SkoNLWwdoP0LA/640?wx_fmt=png)不会随 p (x∣θ) 的缩放而变化。这很容易看出来：
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLpgk0WQMRiaS4LOWMnNwvVeuj1vp35oat6XqaAABUgicSiaVHxMyBIZ4QCw/640?wx_fmt=png)
施加于概率密度的任何比例因子都会消失。因此，如果我们有一个直接对分数估计![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLp81KyNQ1oEPlIHGFa2shAkhDx7NG5wC6ImVwOES7jYndqmromUE1ECA/640?wx_fmt=png)进行参数化的模型，那就可以通过最小化分数匹配损失来拟合分布（而不是直接最大化似然）：
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLpnxRvmhT1h1n9c8LRECialkbBX5ngATlwWfmnUua88xRX3UOB8IXm7Sg/640?wx_fmt=png)
但是，使用这种形式时，损失函数可能不实用，因为我们没有为任意数据点 x 计算基本真值分数![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLpo3DSqwUBINRbNMIiaPR7QElZa80UNibmNM1r6fv65fBM5kWciazTboRsg/640?wx_fmt=png)的好方法。有一些技巧可用来回避这一要求，并将其转换为易于计算的损失函数，包括隐式分数匹配（ISM）、切片分数匹配（SSM）和去噪分数匹配（DSM）。这里我们详细看看最后一种方法：
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLpfJYtcEM5gf4Faiaia8d7KYibMsMm6XPtIv2qBcJIrpK2Iyrg22KRFaX3g/640?wx_fmt=png)
其中，可通过向 x 添加高斯噪声得到![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLp7ibuxzhT9yeJdQgjD77asrVSj2RRmJElLSLhs2DhJNZedOkmLVoS34w/640?wx_fmt=png)。这意味着![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLpmmRhF27DTfxvbXOiawo7efubmiaTB0D2p6quaRo2Xeh7BLkoTJMDSVaA/640?wx_fmt=png)的分布基于一个高斯分布 N (x,σ^2)，其基本真值条件分数函数可以闭式形式计算得到：
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLpmNol9USHvrBhB6VLNunQOFgn09epSsO1JPA2pTRTt77uNCxeO6kibHg/640?wx_fmt=png)
这种形式有非常直观的解释：这是向 x 添加噪声以得到![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLp13Tw8Vt6qhBKUjFgicEtockWYwUcln10Gk2OAA1Ufl03GoSKKM5JBibA/640?wx_fmt=png)的一种扩展版本。因此，通过按照分数（= 对数似然的梯度上升）来使![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLp13Tw8Vt6qhBKUjFgicEtockWYwUcln10Gk2OAA1Ufl03GoSKKM5JBibA/640?wx_fmt=png)的可能性更高这一做法直接对应于消除（部分）噪声：
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLpgFJMibFCax5zCjbD2U0LMyjOQj1G7Pq0bUFxicSk2eoEsUic5LeYqtxHQ/640?wx_fmt=png)

如果我们选择步长 η=σ^2，则就能在一步之内恢复干净数据 x。

L_SM 和 L_DSM 是不同的损失函数，但巧妙之处在于它们具有相同的期望最小值：

![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLphCw0zyyY5f8cklbmxpSpibJggO2cuRzjaGyyeFiczm5z52WPibvlcxhgQ/640?wx_fmt=png)，其中 C 是一个常数。Pascal Vincent 早在 2010 年就推导出了这种等价性，如果你想加深理解，强烈建议你阅读他的技术报告：http://www.iro.umontreal.ca/~vincentp/Publications/smdae_techreport.pdf
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLp11xZq0SGgn0pGUm7T3Tf40kR7VgJQQXqibsG7dgNyrO5fa7y1a9nROA/640?wx_fmt=png)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLpwiaZWHTFsYtld5runPGKoGSfPC4Xw1wucEZnvwOw6bp8LA2NQr70QJg/640?wx_fmt=png)

该方程还描述了一个扩散过程。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLp3CZSAsC3s9VfXHvOhFHzNfUMYVWLVlopGM4bVvej04icY4CuAwjYPdQ/640?wx_fmt=png)是逆向维纳过程，![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLpn4UdXqLQdbeInukFlLUuE0iamtLL0MJGmq73wicYzvrb9WhouW4K57QA/640?wx_fmt=png)是时间依赖型分数函数。这种时间依赖性源自这一事实：噪声水平会随时间变化。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLpXqgb5e3bJgLYzH761wjUKDyJbNrnyRFaicGu8BDiamvd87vgeoCLIs5A/640?wx_fmt=png)
该方程描述了前向和后向过程（只需翻转符号即可变换方向），注意时间依赖型分数函数![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLpMmWc56l8iaLZktK7jgfspd0KvzoZ5QcbL9D91GnWTbGlVImicQeSuWew/640?wx_fmt=png)依然在其中。要证明这一点，你可以写下该 SDE 和概率流 ODE 的福克 - 普朗克方程，然后做一些代数运算，你会发现它们其实是一样的，因此必定具有形同的解 p_t (x)。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLpSUopiakt2bia48xPFOlfV3mXODlficpV5rvzdJVJ7nODicibpPoWCicO4D6g/640?wx_fmt=png)
*展开的扩散采样回路示意图*
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLpGb1FaibDIHqfvq5yEtOkFXtX4HeoibfoSx891pGU7ISHIvd6ft6JZHzA/640?wx_fmt=png)
*8x8 离散余弦变换（比如 JPEG 压缩方法就使用了它）的空间频率分量的可视化*
自然图像以及许多其它自然信号的频域会表现出一个有趣的现象：不同频率分量的幅度往往与频率的倒数成比例下降：![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLpE1AG6GhBhhDLZfB1H7vUEjJK31GF6jVXzDcuo01icxHjRU7goIWxHPw/640?wx_fmt=png)（如果你看的是功率谱而不是幅度谱，那么就是频率平方的倒数）。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KmXPKA19gWibwK2TxMMcXEnEt9e6wOtLpAmibx792Mg7zH37MNzSn4vPJrjJasxx9xm4Gw3ofoEkvicKv9bIOib2gw/640?wx_fmt=png)
*自然图像、高斯噪声和有噪声图像的幅度谱*
